...
def graph_prediction(model, save = None):
    ...
    for i in range(len(TEMPS)):
        ...
        s2 = '%.3f'%meape(x_pred[i], x_obs[i])
        labels.append(' -- $\mathcal{E} =\ $'.join([s1, s2]))
    ...
    ax.legend(framealpha = 1, edgecolor = 'None', loc = 2)
    ...
    ax.set_title('$\mathcal{E} = %.3f$'%meape(y_pred, y_obs))
