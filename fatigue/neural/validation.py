...
def cross_val_eval(Xv, Xc, y, n_epochs, n_batch, \
                   n_folds, c_len = 120, gpu_list = None, load_func = load_known_lstm_model, save_path = None):
    ...
    fold = KFold(n_splits=n_folds, shuffle=True, random_state = 100) 
    ...
    for train, test in fold.split(Xv, y):
        ...
        preprocess_input(Xv_train, Xv_test, Xc_train, Xc_test, y_train, y_test, c_len)
        ...
        print(abs(y_true1-y_pred1)/y_true1*100)
        
        all_y_true += y_true1.tolist()
        all_y_pred += y_pred1.tolist()
        
        rmse = mean_squared_error(y_true1, y_pred1)
...
# =============================================================================
    ...
    for c_len in [1, 10, 20, 40, 60, 80, 100, 120, 150, 200, 500, 1000, 5000, 10000, 20000]:
        t1 = time.time()
        ...
        np.savez('mdata/ydata-18-01-22-%d'%c_len , y_obs=y_true, y_pred=y_pred)
