...
from fatigue.graph.models2 import graph_nn_1m_dev, graph_nn_11_dev, \
...
for i in [48, 30, 37, 36, 33, 34, 31, 40, 39, 16, 28, 22, 4, 50]:
...
# Linear features only {'l1_ratio': 0.886, 'alpha': 0.0011536181017364785}
# Slim features only {'l1_ratio': 0.6, 'alpha': 0.0007275483529196233}

params = {'l1_ratio': 0.6, 'alpha': 0.0007275483529196233}
...
# c, m = graph_nn_1m_dev(r_data, log = log, load = False, which = 'all', ver = True)
...
all_obs = list(np.rint(np.concatenate((d['y_obs_train'].reshape(22,-1),
                        d['y_obs_dev'].reshape(11,-1),
                        d['y_obs_test'].reshape(11,-1)), axis = 0)).flatten().astype('int64'))
all_pred = np.concatenate((d['y_pred_train'].reshape(22,-1),
                          d['y_pred_dev'].reshape(11,-1),
                          d['y_pred_test'].reshape(11,-1)), axis = 0).flatten()
...
# important_feats = r[np.abs(r.coef) >= max_coeff*0.01]
